...
contract MultiChainResolver is AbstractMultiChainResolver {
    ...
    function setChainAddr (bytes32 node, bytes4 chain, string addrValue) public onlyOwner(node) {
    ...
    function chainAddrAndMetadata (bytes32 node, bytes4 chain) public view returns (string, bytes32) {
        ChainAddress storage chainAddress = chainAddresses[node][chain];
        return (chainAddress.addr, chainAddress.metadata);
    }

    function setChainAddrWithMetadata (bytes32 node, bytes4 chain, string addrValue, bytes32 metadataValue) public onlyOwner(node) {
